function init(){
	...
	// CADA MENU
	document.getElementById("btccateno").addEventListener("click",show_menu_arcate,false);
...
function show_menu_arcate(){
	document.getElementById("xm_area_menu").style.display="inline";
	var o_mx_lista = "";
	// armando el listado
	o_mx_lista += '	<select class="listas" id="mx_opc_order"> ';
	o_mx_lista += ' 	<option value = "ccateno">Categoria Id</option> ';
	o_mx_lista += '		<option value = "cdesc">Descripcion</option> ';
	o_mx_lista += '		<option value = "ctypecate">Tipo Categoria</option> ';
	o_mx_lista += '		<option value = "ctypeadj">Comportamiento</option> ';
	o_mx_lista += '	</select> ';
	...
	o_mx_Header += '	<thead> ';
	o_mx_Header += '		<tr> ';
	o_mx_Header += '			<td width="100px"> Categoria Id </td> ';
	o_mx_Header += '			<td width="200px"> Descripcion </td> ';
	o_mx_Header += '			<td width="50px"  > Tipo Categoria </td> ';
	o_mx_Header += '			<td width="50px" title= "E = Entrada, S = Salida"> Comportamiento </td> ';
	o_mx_Header += '		</tr> ';
	o_mx_Header += '	</thead> ';
	o_mx_Header += '</table> ';
...
function get_mx_detalle(){
		...
		if (odata != null){
			...
			for (var i = 0; lnrows > i; ++i){
				...
				o_mx_detalles += '	<td width="100px"> '+ odata[i]["ccateno"]   + '</td> ';
				o_mx_detalles += '	<td width="200px"> '+ odata[i]["cdesc"]     + '</td> ';
				o_mx_detalles += '	<td width="50px" title= "Indica donde se utilizara, A=En Requisas Entradas y Salidas" >'+ odata[i]["ctypecate"] + '</td> ';
				o_mx_detalles += '	<td width="50px" title= "E = Entrada, S = Salida">  '+ odata[i]["ctypeadj"]  + '</td> ';
				o_mx_detalles += '</tr> ';
			}
			o_mx_detalles += '</tbody> ';
		}else{
...
function select_xkey(e){
	...
	document.getElementById("ccateno").value = lckey;
	cerrar_mx_view();
	update_window(lckey);
}
// -----------------------------------------------------------------------
